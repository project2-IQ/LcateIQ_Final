...
async function analyzeProject(projectText) {
  ...
  try {
    ...
    const locDisplay = displayLocationName(lang, location);
    const typeDisplay = displayProjectTypeName(lang, projectType);
    const suitDisplay = formatSuitabilityDisplay(lang, data.suitability);
    const scoreNum = typeof data.score === "number" ? data.score : parseFloat(data.score);
    const scoreStr = Number.isFinite(scoreNum) ? scoreNum.toFixed(2) : String(data.score ?? "");
    const why = whyThisLocationText(lang, data.suitability, scoreNum);
    ...
    const resultText =
      ...
      `${t(lang, "result_location")}: ${locDisplay}\n` +
      `${t(lang, "result_type")}: ${typeDisplay}\n` +
      `${t(lang, "result_suitability")}: ${suitDisplay}\n` +
      `${t(lang, "result_percent")}: ${scoreStr}%\n` +
      coordsLine +
      `${t(lang, "result_map")}`;

    addMessage("bot", resultText);
    updateMapWithResult(data, location, locDisplay);
  } catch (error) {
...
// ============================================
// تحديث الخريطة حسب نتيجة التحليل (موقع واحد)
// ============================================
function updateMapWithResult(data, location, labelLocation) {
  ...
  const dLat = typeof data.latitude === "number" ? data.latitude : parseFloat(data.latitude);
  const dLng = typeof data.longitude === "number" ? data.longitude : parseFloat(data.longitude);
  ...
  const lang = getSavedLang();
  const placeName = labelLocation || location;
  const suitLabel = formatSuitabilityDisplay(lang, data.suitability);
  const label = `${suitLabel} (${data.score}%)`;
